...
def dashboard():
    ...
    for budget in budgets:
        ...
        # Use the spent_amount field from the budget model
        spent_amounts.append(budget.spent_amount if budget.spent_amount is not None else 0.0)
    
    total_spent = sum(spent_amounts) if spent_amounts else 0
    ...
    total_current_value = sum(investment.current_value for investment in investments if investment.current_value is not None) if investments else 0
    ...
    # Ensure all lists are not empty to avoid JSON serialization issues
    if not expense_categories:
        expense_categories = ['No Data']
        expense_amounts = [0]
    
    if not budget_categories:
        budget_categories = ['No Data']
        budget_amounts = [0]
        spent_amounts = [0]
